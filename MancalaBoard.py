...
class MancalaBoard:
    ...
    def reset(self):
        ...
        self.repeated_turn = False
        self.last_move = 0

    def clone(self):
        board = MancalaBoard()
        board.scoreCups = deepcopy(self.scoreCups)
        board.P1Cups = deepcopy(self.P1Cups)
        board.P2Cups = deepcopy(self.P2Cups)
        return board

    def getScore(self, playerNum):
        return self.scoreCups[playerNum - 1]
    ...
    def makeMoveHelp( self, player, cup ):
        ...
        if playAgain:
            self.repeated_turn = True
            return True
        else:
            self.repeated_turn = False

        # Now see if we ended in a blank space on our side
        if cups == initCups and cups[cup-2] == 1 and \
