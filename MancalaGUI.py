...
def startGame(p1, p2, game_num):
    """ Start the game of Mancala with two players """

    root = Tk()
    app = MancalaWindow(root, p1, p2, True)

    root.mainloop()


def startGameNoGUI(p1, p2, game_num):
    """ Start the game of Mancala with two players """

    root = Tk()
    app = MancalaWindow(root, p1, p2, False)
    app.root.withdraw()
    app.newgame()
    # root.mainloop()
    if app.won == 0:
        # print("Game n.{0}: There was a tie".format(str(game_num+1)))
        return 0
    else:
        # print("Game n.{0}: Player {1} won".format(str(game_num+1), str(app.won.num)))
        return app.won.num